...
class BlockSetParser {
  ...
  private static Optional<IBlockFactory> parseFloor(JsonObject json) throws DungeonSettingParseException {
    return json.has("floor")
        ? ofNullable(BlockProvider.create(json.get("floor").getAsJsonObject()))
        : empty();
  }

  private static Optional<IBlockFactory> parseWalls(JsonObject json) throws DungeonSettingParseException {
    return json.has("walls")
        ? ofNullable(BlockProvider.create(json.get("walls").getAsJsonObject()))
        : empty();
  }

  private static Optional<IStair> parseStair(JsonObject json) throws DungeonSettingParseException {
    return json.has("stair")
        ? of(somethingAboutStairWithData(json))
        : empty();
  }

  private static MetaStair somethingAboutStairWithData(JsonObject json) throws DungeonSettingParseException {
  ...
  private static Optional<IBlockFactory> parsePillar(JsonObject json) throws DungeonSettingParseException {
    return json.has("pillar")
        ? ofNullable(BlockProvider.create(json.get("pillar").getAsJsonObject()))
        : empty();
  }

  private static Optional<IDoor> parseDoor(JsonObject json) throws DungeonSettingParseException {
    return json.has("door")
        ? of(new Door(json.get("door")))
        : empty();
  }

  private static Optional<IBlockFactory> parseLightBlock(JsonObject json) throws DungeonSettingParseException {
    return json.has("lightblock")
        ? ofNullable(BlockProvider.create(json.get("lightblock").getAsJsonObject()))
        : empty();
  }

  private static Optional<IBlockFactory> parseLiquid(JsonObject json) throws DungeonSettingParseException {
