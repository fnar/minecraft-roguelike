...
public class ToolLootItemParser {
  ...
  public static ToolLootItem parse(JsonObject data, int weight) throws Exception {
    return new ToolLootItem(
        weight,
        parseLevel(data),
        parseEquipmentType(data),
        parseQuality(data),
  ...
  private static Equipment parseEquipmentType(JsonObject data) throws Exception {
