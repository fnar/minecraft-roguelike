...
public class DungeonPyramidCorner extends DungeonBase {
  ...
  public DungeonBase generate(WorldEditor editor, LevelSettings settings, Coord origin, List<Cardinal> entrances) {
    ...
    RectSolid.fill(editor, new Coord(x - 2, y + 3, z - 2), new Coord(x + 2, y + 5, z + 2), blocks, false, true);
    RectSolid.fill(editor, new Coord(x - 1, y + 3, z - 1), new Coord(x + 1, y + 3, z + 1), SingleBlockBrush.AIR, true, true);

    // floor
    RectSolid.fill(editor, new Coord(x - 3, y - 1, z - 3), new Coord(x + 3, y - 1, z + 3), theme.getPrimary().getFloor(), false, true);
